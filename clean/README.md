# Clean Architecture - BookStore

## Yêu cầu
- Python 3.8+
- MySQL 8.0+

## Cài đặt & Chạy

### 1. Tạo môi trường ảo
```bash
python3 -m venv venv
source venv/bin/activate
```

### 2. Cài đặt dependencies
```bash
pip install -r requirements.txt
```

### 3. Tạo database MySQL
```sql
CREATE DATABASE clean_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

### 4. Cấu hình database
Sửa file `framework/bookstore_clean/settings.py`:
```python
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.mysql',
        'NAME': 'clean_db',
        'USER': 'root',
        'PASSWORD': 'your_password',  # Thay đổi
        'HOST': 'localhost',
        'PORT': '3306',
    }
}
```

### 5. Chạy migrations
```bash
cd framework
python manage.py makemigrations
python manage.py migrate
```

### 6. Tạo superuser
```bash
python manage.py createsuperuser
```

### 7. Chạy server
```bash
python manage.py runserver 8001
```

Truy cập: http://localhost:8001/

## Cấu trúc thư mục
```
clean/
├── domain/                       # Domain Layer - Entities
│   ├── __init__.py
│   └── entities.py
│
├── usecases/                     # Use Cases Layer - Business Logic
│   ├── __init__.py
│   ├── customer_usecases.py
│   └── book_usecases.py
│
├── interfaces/                   # Interfaces Layer - Abstractions
│   ├── __init__.py
│   └── repositories.py
│
├── infrastructure/               # Infrastructure Layer - Repository Implementations
│   ├── __init__.py
│   ├── django_models.py
│   └── django_repositories.py
│
└── framework/                    # Framework Layer - Django Web
    ├── manage.py
    ├── bookstore_clean/          # Django project settings
    │   ├── settings.py
    │   ├── urls.py
    │   └── wsgi.py
    ├── web/                      # Django app
    │   ├── views.py
    │   ├── urls.py
    │   └── admin.py
    └── templates/                # HTML templates
        ├── base.html
        ├── home.html
        ├── login.html
        ├── register.html
        ├── books.html
        └── cart.html
```

## URLs chính
| URL | Mô tả |
|-----|-------|
| `/` | Trang chủ |
| `/accounts/register/` | Đăng ký |
| `/accounts/login/` | Đăng nhập |
| `/books/` | Danh sách sách |
| `/cart/` | Giỏ hàng |
| `/admin/` | Django Admin |
